...
public class PNGMovementController : MonoBehaviour
{
    ...
    private PNGMovementController _motor;
    ...
    public float slowfallSpeed = 16f;
    ...
    private void Start()
    {
        ...
        _motor = GetComponent<PNGMovementController>();
    }

    void FreedomStateSave(PNGMovementController motor)
    {
    ...
    void FreedomStateRestore(PNGMovementController motor)
    {
        if (_restored) // do not enter twice
            return;
    ...
    void PlatformerMotor()
    {
        // use last state to restore some ladder specific values
        if (_motor.motorState != PNGMovementController.MotorState.FreedomState)
        {
        ...
        if (UnityEngine.Input.GetButtonDown(PlayerInput.JUMP))
        {
            _motor.Jump();
        ...
        {
            _motor.fallSpeed = slowfallSpeed; 
        } else
        {
            _motor.fallSpeed = fastfallSpeed;
        }

        _motor.jumpingHeld = UnityEngine.Input.GetButton(PlayerInput.JUMP);

        // XY freedom movement
        if (_motor.motorState == PNGMovementController.MotorState.FreedomState)
        {
            _motor.normalizedXMovement = UnityEngine.Input.GetAxis(PlayerInput.HORIZONTAL);
            _motor.normalizedYMovement = UnityEngine.Input.GetAxis(PlayerInput.VERTICAL);

            return; // do nothing more
        }

        // X axis movement
        if (Mathf.Abs(UnityEngine.Input.GetAxis(PlayerInput.HORIZONTAL)) > Globals.INPUT_THRESHOLD)
        {
            _motor.normalizedXMovement = UnityEngine.Input.GetAxis(PlayerInput.HORIZONTAL);
        }
        else
        ...
        if (UnityEngine.Input.GetAxis(PlayerInput.VERTICAL) != 0)
        {
            bool up_pressed = UnityEngine.Input.GetAxis(PlayerInput.VERTICAL) > 0;
            if (_motor.IsOnLadder())
            {
                if (
                    (up_pressed && _motor.ladderZone == PNGMovementController.LadderZone.Top)
                    ||
                    (!up_pressed && _motor.ladderZone == PNGMovementController.LadderZone.Bottom)
                 )
                {
                    // do nothing!
                }
        ...
        else if (UnityEngine.Input.GetAxis(PlayerInput.VERTICAL) < -Globals.FAST_FALL_THRESHOLD)
        {
            _motor.fallFast = false;
        }

        if (UnityEngine.Input.GetButtonDown(PlayerInput.DASH))
        {
            _motor.Dash();
        }
    }
    ...
    public class Globals
    {
        ...
        public const float FAST_FALL_THRESHOLD = 0.1f;
        public const int ENV_MASK = 0x100;
        public const string PACKAGE_NAME = "Zetcil";
