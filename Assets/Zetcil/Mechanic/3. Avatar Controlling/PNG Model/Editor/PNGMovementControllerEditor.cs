...
[CustomEditor(typeof(PNGMovementController))]
[CanEditMultipleObjects]
public class PNGMovementControllerEditor : Editor
{
    ...
    private readonly Property SLOWFALL_SPEED = new Property("slowfallSpeed", "Slow Fall Speed");
    private readonly Property FASTFALL_SPEED = new Property("fastfallSpeed", "Fast Fall Speed");
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (_showMovement)
        {
            ...
            DisplayRateField(SLOWFALL_SPEED);
            DisplayRateField(FASTFALL_SPEED);
    ...
    private void CheckAndDisplayInfo()
    {
        ...
        */
        ...
        if (!_properties[STATIC_ENV_LAYER_MASK.name].hasMultipleDifferentValues &&
            (_properties[STATIC_ENV_LAYER_MASK.name].intValue & (1 << ((PNGMovementController)target).gameObject.layer)) != 0)
        {
            EditorGUILayout.HelpBox(
    ...
    private void CheckAndDisplayTimingWarnings(Property property)
    {
                ...
                Mathf.Round(_properties[property.name].floatValue / Time.fixedDeltaTime)))
        {
            string msg = string.Format(
                ...
                property.text,
                Time.fixedDeltaTime,
                PNGMovementController.Globals.GetFrameCount(_properties[property.name].floatValue) * Time.fixedDeltaTime,
    ...
    private void DisplayTimingField(Property property)
    {
        ...
        if (!_properties[property.name].hasMultipleDifferentValues)
        {
            frameCount = PNGMovementController.Globals.GetFrameCount(_properties[property.name].floatValue).ToString();
        }
    ...
    private void CheckValues()
    {
        if (!_properties[ENV_CHECK_DISTANCE.name].hasMultipleDifferentValues &&
            _properties[ENV_CHECK_DISTANCE.name].floatValue <= PNGMovementController.Globals.MINIMUM_DISTANCE_CHECK * 2)
        {
            _properties[ENV_CHECK_DISTANCE.name].floatValue = PNGMovementController.Globals.MINIMUM_DISTANCE_CHECK * 2;
        }

        if (!_properties[MIN_DISTANCE_FROM_ENV.name].hasMultipleDifferentValues &&
            _properties[MIN_DISTANCE_FROM_ENV.name].floatValue <= PNGMovementController.Globals.MINIMUM_DISTANCE_CHECK)
        {
            _properties[MIN_DISTANCE_FROM_ENV.name].floatValue = PNGMovementController.Globals.MINIMUM_DISTANCE_CHECK;
        }
    ...
    private float GetJumpDistance()
    {
        return _properties[AIR_SPEED.name].floatValue * 2 *
               Mathf.Sqrt(2 *
                    ...
                    (((PNGMovementController)target).gravityMultiplier *
